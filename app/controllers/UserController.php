...
class UserController extends ControllerBase
{
  public function optionAction() {
    $userId = Hl::user('id');
    if ($userId > 0)
    foreach ($this->request->get() as $kp=>$vp) {
      if ($kp != '_url') {
        $qOpt = HT::sel('options',['name'=>$kp]);
        if (count($qOpt) > 0) {
          return HT::ins('user_options',
            ['option_id'=>$qOpt[0]['id'],'user_id'=>$userId,'val'=>$vp,
          //'query'=>1,
          ],true);
        }
      }
  ...
  {
    $this->response->redirect(array('action' => 'login'));
  }
  ...
  public function listAction() {
    ...
    if ($this->request->isPost()) {
      $query = Criteria::fromInput($this->di, 'Hotels', $_POST);
    ...
    $mUser = User::find($parameters);
    if (count($mUser) == 0) {
      $this->flash->notice("The search did not find any hotels");
  ...
  public function edit0Action($id=null)
  {
   ...
   * Register user
   */
  public function register000Action()
  {
  ...
  {
echo 'profile';
    var_dump($this->auth->getIdentity()['name']);
  }
}